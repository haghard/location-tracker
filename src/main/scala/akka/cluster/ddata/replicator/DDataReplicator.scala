...
object DDataReplicator {
  ...
  ) = Props(new DDataReplicator(sharedMemoryMap, settings)).withDispatcher(settings.dispatcher)
}
...
final class DDataReplicator(
  ...
  def setData(key: KeyId, envelope: DataEnvelope): DataEnvelope = {
    val digest0 =
      if (envelope.data == DeletedData) DDataReplicator.DeletedDig
      else {
  ...
  def getLocalDigest(key: KeyId): Array[Byte] =
    Option(sharedMemoryMap.get(vehicleId2Long(key))) match {
      case Some(v) =>
        ...
        v.digest
      case None => DDataReplicator.NotFoundDig
    }
  ...
  def receiveGossipTick(): Unit = {
    if (ThreadLocalRandom.current().nextDouble() > 0.6) {
      ...
      /*s"\nMemory [Total:${sharedMemoryMap.getTotalMemory}, Used: ${sharedMemoryMap.getUsedMemory}] MaxCapacity:${sharedMemoryMap.getCapacity} Count:${sharedMemoryMap.getCount}\n" +
          s"Storage [Free ${(sharedMemoryMap.getFreeMemory * 100) / sharedMemoryMap.getTotalMemory}% | Used ${(sharedMemoryMap.getUsedMemory * 100) / sharedMemoryMap.getTotalMemory}%]"*/
    }
  ...
  def receiveStatus(thatDigests: Map[KeyId, Digest], fromSystemUid: Option[Long]): Unit = {
    ...
    def digestStatus(key: KeyId, otherDigest: Array[Byte]): DDataReplicator.DigestStatus = {
      val digest = getLocalDigest(key)

      val notFound = java.util.Arrays.equals(digest, DDataReplicator.NotFoundDig)
      if (!notFound && !java.util.Arrays.equals(digest, otherDigest)) DDataReplicator.DigestStatus.Different
    ...
    thatDigests.foreach { case (key, digest) =>
      digestStatus(key, digest.toArrayUnsafe()) match {
        case DDataReplicator.DigestStatus.Same      =>
        case DDataReplicator.DigestStatus.Different => differentKeys += key
        case DDataReplicator.DigestStatus.NotFound  => missingKeys += key
      }
    }
  ...
  def receiveGossip(updatedData: Map[KeyId, DataEnvelope], sendBack: Boolean, fromSystemUid: Option[Long]): Unit = {
    ...
    updatedData.foreach { case (key, thatEnvelope) =>
      ...
      val merged = that.markSeen(
        selfUniqueAddress,
        Try(prev.envelope.asInstanceOf[DataEnvelope].data.asInstanceOf[ReplicatedVehicle]).toOption
      )
